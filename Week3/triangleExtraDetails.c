...
//Sorts an array of integers through a bubble sorting method
void bubbleSort(int max, long sortArray[]) {
    long temp;
    bool swapFlag = true;

    while (max != 0 && swapFlag) {
      swapFlag = false;
...
//Sorts an array of doubles through a bubble sorting method
void bubbleSortDoub(int max, long double sortArray[]) {
    long double temp;
    bool swapFlag = true;
...
/*
I've left this code in my program but unused, as while it can find the
centre of mass for every triangle provided except the scalene and flat, due to
the way the program intiially takes input it would be quite difficult to relate
the centre of mass to the given measurements in any meaningful way.
*/
void centOfMass(const int type, long double coords[2], const long double s1, const long double s2, const long double s3) {
    long double height;
    long double side[] = {s1, s2, s3};
    bubbleSortDoub(3, side);
    ...
    if (type == Isosceles) {
      if (side[0] == side[1]) {
        height = (2 * area(s1,s2,s3)) / side[2];
        coords[0] = side[2] / 2;
      }
      if (side[1] == side[2]) {
        height = (2 * area(s1,s2,s3)) / side[0];
        coords[0] = side[0] / 2;
      }
      if (side[0] == side[2]) {
        height = (2 * area(s1,s2,s3)) / side[1];
        coords[0] = side[1] / 2;
      }
      coords[1] = height / 3;
    }
    ...
    if (type == Scalene) {
      long double angle;
      angle = relatedAngle(side[0], side[1], side[2]);
      height = sinl(angle);
    }
}
