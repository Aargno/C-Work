...
void extendedArg(byte *cs, int num, int out) {
  switch (num) {
    case 1: out = (int8_t)cs[0];
    break;
    case 2: out = (int16_t)packTwoBytes(cs[0], cs[1]);
    break;
    case 3: out = (int32_t)packFourBytes(cs[0], cs[1], cs[2], cs[3]);
    break;
  }
}
...
// int interpret8Bit(byte c, bool tComp) {
//     if (!tComp) return (c & 0x3F);
//     else {
//       int num = c & 0x7F;
//       byte d = (c >> 7) & 0x01;
...
// }
//
// int interpret16Bit(twoByte c) {
//       int num = c & 0x7FFF;
//       twoByte d = (c >> 15) & 0x0001;
//       if (d == 0x0001) num -= 128;
//       return num;
//     }
// }
//
// int interpret32Bit(fourByte c, bool tComp) {
//     if (!tComp) return (c & 0x7F);
//     else {
//       int num = c & 0x1F;
//       byte d = (c >> 5) & 0x01;
//       if (d == 0x01) num -= 32;
...
int extendHandler(byte *op, int *arg, byte *cs, int i) { //returns value i is to be incremented by in control
  int opBytes = retrieveByteNum(cs[i]);
  byte *extend;
  op = extendedOp(cs[i]);
  switch (opBytes) {
    case 0: return 0;
    ...
    case 1: extend = malloc(sizeof(byte));
            extend[0] = cs[i+1];
            extendedArg(extend, 2, (int8_t)arg);
            return 1;
    break;
    case 2: extend = malloc(sizeof(byte) * 2);
            extend[0] = cs[i+1];
            extend[1] = cs[i+2];
            extendedArg(extend, 2, (int16_t)arg);
            return 2;
    break;
    case 3: extend = malloc(sizeof(byte) * 4);
            ...
            extend[2] = cs[i+3];
            extend[3] = cs[i+4];
            extendedArg(extend, 3, (int32_t)arg);
            return 4;
    break;
}
...
void call(byte op, int arg, state *s, display *d) {
  switch (op) {
    ...
    break;
    case 0x80: pause(d, arg);
    break;
    case 0xC0: s->pen = !s->pen;
    break;
    case 0xC4: clear(d);
    break;
    case 0xC5: key(d);
...
void control(byte *c, int len, display *d) {
  ...
  for (int i = 0; i < len; i++) {
    op = retrieveOp(c[i]);
    if (op == 0xC0) i += extendHandler(&op, &arg);
    else arg = retrieveArg(c[i], op);
    call(op, arg, s, d);
  }
}
