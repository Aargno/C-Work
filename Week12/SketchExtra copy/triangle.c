...
void drawTriangle(const char sa[], const char sb[], const char sc[]) {
  int s1, s2, s3;
  s1 = atoi(sa);
  s2 = atoi(sb);
  s3 = atoi(sc);
  if (s1 <= 200 && s2 <= 200 && s3 <= 200) {
    displayTriangle(s1, s2, s3);
  }
}
...
int triangle(const char sa[], const char sb[], const char sc[]) {
    ...
    if (overflowPotential(sa,sb,sc)) {
      ...
      int result = intTriCheck(sa,sb,sc);
      if (result != Impossible && result != Illegal) drawTriangle(sa, sb, sc);
      return result;
    }
}
