...
//-----------------------------------------------------------------------------//
//Display functions (only new functionality added)

void drawBoard(display *d) {
  line(d, 20, 70, 180, 70);
  pause(d, 63);
  line(d, 70, 30, 70, 160);
  pause(d, 63);
  line(d, 120, 30, 120, 160);
  pause(d, 63);
  line(d, 20, 120, 180, 120);
  pause(d, 63);
}

//Draws an octagon, starting from the provided coords
//x0/y0 and x1/y1 alternate postions on each step, while there are
//some redundant assignments these have been included in the interest of making
//algorithm easier to follow.
void drawOct(display *d, int x0, int y0) {
  int x1, y1;
  x1 = x0;
  y1 = y0 + 10;
  line(d, x0, y0, x1, y1);
...
void drawCross(display *d, int x, int y) {
  line(d, x, y, (x+30), (y+20));
  line(d, x, (y+20), (x+30), y);
}

void drawGame(display *d, game *state) {
  int x = 30;
  int y = 40;
  for (int i = 0; i < 3; i++) {
    for (int j = 0; j < 3; j++) {
      if (state->grid[i][j] == X) drawCross(d, x, y);
      else if (state->grid[i][j] == O) drawOct(d, x, y);
      x += 50;
    }
    x = 30;
    y += 50;
...
player lineCheck(player x, player y, player z) {
    if (x == y && y == z) return z;
    return N;
}
...
player won(game *g) {
    if (lineCheck(g->grid[0][0], g->grid[0][1], g->grid[0][2]) != N) return lineCheck(g->grid[0][0], g->grid[0][1], g->grid[0][2]);
    if (lineCheck(g->grid[1][0], g->grid[1][1], g->grid[1][2]) != N) return lineCheck(g->grid[1][0], g->grid[1][1], g->grid[1][2]);
    if (lineCheck(g->grid[2][0], g->grid[2][1], g->grid[2][2]) != N) return lineCheck(g->grid[2][0], g->grid[2][1], g->grid[2][2]);
    if (lineCheck(g->grid[0][0], g->grid[1][0], g->grid[2][0]) != N) return lineCheck(g->grid[0][0], g->grid[1][0], g->grid[2][0]);
    if (lineCheck(g->grid[0][1], g->grid[1][1], g->grid[2][1]) != N) return lineCheck(g->grid[0][1], g->grid[1][1], g->grid[2][1]);
    if (lineCheck(g->grid[0][2], g->grid[1][2], g->grid[2][2]) != N) return lineCheck(g->grid[0][2], g->grid[1][2], g->grid[2][2]);
    if (lineCheck(g->grid[0][0], g->grid[1][1], g->grid[2][2]) != N) return lineCheck(g->grid[0][0], g->grid[1][1], g->grid[2][2]);
    if (lineCheck(g->grid[0][2], g->grid[1][1], g->grid[2][0]) != N) return lineCheck(g->grid[0][2], g->grid[1][1], g->grid[2][0]);
    return N;
}
...
void printRow(game *g, int row) {
    printf(" %c | %c | %c", showPlay(g->grid[row][0]), showPlay(g->grid[row][1]), showPlay(g->grid[row][2]));
    printf("\n");
}

// Display the grid.
void show(display *d, game *g) {
  drawBoard(d);
  drawGame(d, g);
}
...
void playAI(char player) {
    bool finish = false;
    display *d = newDisplay("oxo", 200, 200);
    game g;
    newGame(&g, player);
...
void testLine() {
    assert(lineCheck(X, X, X) == X);
    assert(lineCheck(O, O, O) == O);
    assert(lineCheck(X, O, O) == N);
    assert(lineCheck(O, X, O) == N);
    assert(lineCheck(O, O, X) == N);
    assert(lineCheck(N, N, N) == N);
    assert(lineCheck(X, N, N) == N);
    assert(lineCheck(O, O, N) == N);
}
