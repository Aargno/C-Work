...
fourByte packFourBytes(byte a, byte b, byte c, byte d) {
  ...
  out = ((a << 24) & 0xFF000000) | out;
  out = ((b << 16) & 0x00FF0000) | out;
  out = ((c << 8) & 0x0000FF00) | out;
  out = (d & 0x000000FF) | out;
  return out;
}
...
int handleArg(int opBytes, byte cs[], int i) { //Returns argument in extended bytes
  byte *extend;
  switch (opBytes) {
    case 0: return 0;
    break;
...
//Separated to make control function a bit less complex
int extendControl(int op, int arg, int i, state *s, display *d, byte *c) {
  int opBytes = retrieveByteNum(c[i]);
  ...
  return incrementHandle(opBytes); //Returns number array index is to be incremented by
}
...
void control(byte *c, int len, display *d) {
  ...
  for (int i = 0; i < len; i++) {
    op = retrieveOp(c[i]);
    if (op == 0xC0) {
      i += extendControl(op, arg, i, s, d, c);
    }
    else {
...
void testExtendArg() {
  byte cs[] = {0xFF,0xFF,0xFF,0xFF};
  assert(extendedArg(cs, 1) == -1);
  assert(extendedArg(cs, 2) == -1);
  assert(extendedArg(cs, 3) == -1);
}

void testPackTwo() {
  assert(packTwoBytes(0x00, 0xFF) == 0x00FF);
  assert(packTwoBytes(0xF0, 0x0F) == 0xF00F);
  assert(packTwoBytes(0x0D, 0xFC) == 0x0DFC);
  assert(packTwoBytes(0x00, 0x00) == 0x0000);
}
